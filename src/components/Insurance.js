      ...
      <div className="container mx-auto px-lg">
        ...
        <div className="flex flex-wrap justify-center items-center gap-xl opacity-60 dark:opacity-80 grayscale dark:invert-[0.1] hover:grayscale-0 dark:hover:invert-0 hover:opacity-100 dark:hover:opacity-100 transition-all duration-300">
          {partners.map((partner, idx) => (
            <div key={idx} className="flex items-center gap-sm font-bold text-headline-sm text-on-surface dark:text-white select-none whitespace-nowrap">
              <span className="material-symbols-outlined text-[28px] text-primary dark:text-primary-fixed-dim flex-shrink-0">
                {partner.icon}
              </span> 
              {partner.name}
            </div>
